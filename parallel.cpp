...
void buildLocSplitMPI(Problem& q, int lrep, int crep)
{
  ...
  //NPROWS = l;  /* number of rows in _decomposition_ */
  //NPCOLS = c;  /* number of cols in _decomposition_ */
  ...
  MPI_Scatter(q.v.data(), NPv, MPI_DOUBLE, q.v.data(), NPv, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  
  q.A.conservativeResize(BLOCKROWS,BLOCKCOLS);
  q.v.conservativeResize(NPblock);
  //q.v0.conservativeResize(N);
  // q.Lmbd.conservativeResize(N);
